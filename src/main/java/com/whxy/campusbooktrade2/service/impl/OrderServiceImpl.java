    ...
    public R<String> createOrder(OrderInfo orderInfo) {
        try {
            ...
            if (save) {
                ...
                if (book != null) {book.setStatus(0); bookService.updateById(book);// 0=已下架
                }
                return R.ok("下单成功！订单号：" + orderNo);
            } else {return R.fail("下单失败");}
        } catch (Exception e) {e.printStackTrace();return R.fail("系统异常");}
    }
    @Override
    public R<List<OrderVo>> getMyOrders(Long userId) {
        List<OrderVo> orderList = baseMapper.selectMyOrdersWithBookName(userId);return R.ok(orderList);
    }
}